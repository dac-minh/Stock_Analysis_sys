with open(r'D:\project\web_ptich_ck\FE\components\news\SectorPerformanceWidget.tsx', 'w', encoding='utf-8') as f:
    f.write('''\'use client\';
...
const SectorPerformanceWidget = () => {
    ...
    useEffect(() => {
        const fetchSectors = async () => {
            setLoading(true);
            try {
                const response = await fetch(\http://localhost:8000/api/v1/news/sector-sentiment?time_range=\\);
                if (response.ok) {
        ...
        <div className=\"p-4 border rounded-xl bg-card shadow-sm\">
            <div className=\"flex items-center justify-between mb-4\">
                ...
                >
                    ...
                    <option value=\"week\">Tu?n này</option>
            ...
            {loading ? (
                ...
                <p className=\"text-sm text-gray-500 text-center py-4\">Chua có d? li?u</p>
            ) : (
